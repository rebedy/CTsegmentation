...
class Unet3D(nn.Module):
    ...
    def __init__(self, n_channels, n_classes, model_depth=4, final_activation="sigmoid"):
        super(Unet3D, self).__init__()
        self.n_classes = n_classes
        self.encoder = EncoderBlock(in_channels=n_channels, model_depth=model_depth)
        self.decoder = DecoderBlock(out_channels=n_classes, model_depth=model_depth)
    ...
    def forward(self, x):
        ...
        x = self.sigmoid(x)
        print(">>> Final UNet output shape: ", x.shape)
        return x
